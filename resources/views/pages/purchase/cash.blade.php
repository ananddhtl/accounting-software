...
<section class="main">
    <div class="middle-dashboard">
        <div class="topic-heading" style="display: inline-flex;">
            ...
            <h1> Purchase and Expenses <small>(Cash Sales)</small></h1>
            @if(session('message'))
        ...
        @if(isset($cashonPurchase))
        <form action="{{ url('/salesonPurchaseCashUpdate',$cashonPurchase->id) }}" method="POST">
            @csrf
            <div class="table-heading">
                <table>
                    ...
                    <tr>
                        <td><input type="text" placeholder="S.N" value="{{ $cashonPurchase->id }}" readonly></td>
                        <td>
                            <input type="hidden" name="accno" value="{{ $cashonPurchase->accno }}" placeholder="Enter Name"
                                required>
                            <input type="text" id="partyName" onkeyup="searchPartyName()" placeholder="Enter Name"
                                value="{{ $cashonPurchase->acname }}" required>
                            <div class="dropdown-content" id="partyname_data"></div>
                        </td>
                        <td><input type="text"  name="particulars" placeholder="Particulars"
                                value="{{$cashonPurchase->particulars}}" required></td>
                    </tr>
                </table>
            </div>
            <div class="table-heading">
        ...
        @else
        <form action="{{ url('/cashOnPurchaseStoreData') }}" method="POST">
            @csrf
            <div class="table-heading">
                <table>
                    <tr>
        ...
        <div class="table-heading">
            <div class="search-form">
                <form method="GET" action="{{ url('/purchaseoncash') }}">
                    @csrf
            ...
            <table>
                ...
                <tr>
                    ...
                    <td><a href="{{url('salesoncashPurchase/'.$item->id)}}"><button class="edit-button">Edit <i
                                    class="ri-pencil-line"></i> </button></a>
                        </button> <button class="del-button"
                            onclick="confirmDelete('{{ url('/deletecashonPurchase/'.$item->id) }}')">Del <i
