...
<section class="main">
    <div class="middle-dashboard">
        <div class="topic-heading" style="display: inline-flex;">
            ...
            <h1> Purchase and Expenses <small>(Credit Sales)</small></h1>
            @if(session('message'))
        ...
        @if(isset($creditonPurchase))
        <form action="{{ url('/salesoncreditUpdatePurchase',$creditonPurchase->id) }}" method="POST">
            @csrf
            <div class="table-heading">
                <table>
                    ...
                    <tr>
                        <td><input type="text" placeholder="S.N" value="{{ $creditonPurchase->id }}"readonly></td>
                        <td>
                            <input type="hidden" name="accno" value="{{ $creditonPurchase->accno }}"
                                placeholder="Enter Name" required>
                            <input type="text" id="partyName" onkeyup="searchPartyName()" placeholder="Enter Name"
                                value="{{ $creditonPurchase->acname }}" required>
                            <div class="dropdown-content" id="partyname_data"></div>
                        </td>
                        <td><input type="text" name="particulars" placeholder="Particulars"
                                value="{{$creditonPurchase->particulars}}" required></td>
                    </tr>
                </table>
            </div>
            <div class="table-heading">
                <table>
                    ...
                    <tr>
                        <td><input type="number" name="amount" placeholder="Enter Amount"
                                value="{{$creditonPurchase->amount}}" required></td>
                        <td><input type="number" name="taxable_amount" placeholder="Enter Tax Amount"
                                value="{{$creditonPurchase->taxable_amount}}" required></td>
                        <td><input type="number" name="vat_amount" placeholder="Enter VAT Amount"
                                value="{{$creditonPurchase->vat_amount}}" required></td>
                    </tr>
                </table>
                <div class="submit">
        ...
        @else
        <form action="{{ url('/creditOnPurchaseStoreData') }}" method="POST">
            @csrf
            <div class="table-heading">
                <table>
                    <tr>
        ...
        <div class="table-heading">
            <div class="search-form">
                <form method="GET" action="{{ url('/purchaseoncredit') }}">
                    @csrf
            ...
            <table>
                ...
                <tr>
                    ...
                    <td><a href="{{url('salesoncreditonPurchase/'.$item->id)}}"><button class="edit-button">Edit <i
                                    class="ri-pencil-line"></i> </button></a>
                        </button> <button class="del-button"
                            onclick="confirmDelete('{{ url('/deletecreditonPurchase/'.$item->id) }}')">Del <i
