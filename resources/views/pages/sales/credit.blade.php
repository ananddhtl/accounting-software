...
<section class="main">
    <div class="middle-dashboard">
        <div class="topic-heading" style="display: inline-flex;">
            ...
            <h1> Sales and Income <small>(Credit Sales)</small></h1>
            @if(session('message'))
        ...
        @if(isset($creditonSales))
        <form action="{{ url('/salesonCreditUpdate',$creditonSales->id) }}" method="POST">
            @csrf
            <div class="table-heading">
                <table>
                    ...
                    <tr>
                        <td><input type="text" placeholder="S.N" value="{{ $creditonSales->id }}" readonly></td>
                        <td>
                            <input type="hidden" name="accno" value="{{ $creditonSales->accno }}" placeholder="Enter Name"
                                required>
                            <input type="text" id="partyName" onkeyup="searchPartyName()" placeholder="Enter Name"
                                value="{{ $creditonSales->acname }}" required>
                            <div class="dropdown-content" id="partyname_data"></div>
                        </td>
                        <td><input type="text"  name="particulars" placeholder="Particulars"
                                value="{{$creditonSales->particulars}}" required></td>
                    </tr>
                </table>
            </div>
            <div class="table-heading">
        ...
        @else
        <form action="{{ url('/creditOnSalesStoreData') }}" method="POST">
            @csrf
            <div class="table-heading">
                <table>
                    <tr>
        ...
        <div class="search-form">
                <form method="GET" action="{{ url('/salesoncredit') }}">
                    @csrf
            ...
            <table>
                ...
                <tr>
                    ...
                    <td><a href="{{url('salesoncredit/'.$item->id)}}"><button class="edit-button">Edit <i
                                    class="ri-pencil-line"></i> </button></a>
                        </button> <button class="del-button"
                            onclick="confirmDelete('{{ url('/deletecreditonSales/'.$item->id) }}')">Del <i
